...
$script:captureActive=$false;$script:captureProcess='';$script:captureProcesses=@();$script:captureStartedUtc=[DateTime]::UtcNow;$script:captureBaseline=@{};$script:captureRows=@{};$script:liveRows=@();$script:liveProcessMap=@{};$script:liveGroupsFingerprint=''
$showLiveSelection={
    ...
    if($process -and $liveGrid.Rows.Count){$liveGrid.ClearSelection();foreach($row in @($liveGrid.Rows)){if(-not $row.IsNewRow){$row.Selected=$true}}}
}
...
New-Button $liveTop 'Запись действия' 595 8 150 {$r=Get-SelectedRow $liveGrid;$script:captureProcess=$r.Process;$script:captureProcesses=$(if($r.Process -in @('steam','steamwebhelper')){@('steam','steamwebhelper')}else{@($r.Process)});$script:captureStartedUtc=[DateTime]::UtcNow;$script:captureBaseline=@{};$script:captureRows=@{};$script:captureActive=$true;$liveStatus.Text="Запись $($script:captureProcess): выполните нужное действие"}|Out-Null
New-Button $liveTop 'Стоп записи' 760 8 130 {$script:captureActive=$false;$captured=@(InvokeAdaptiveScenarioConnections -Processes $script:captureProcesses -SinceUtc $script:captureStartedUtc);if($captured.Count){Sync-LiveGrid $liveGrid $captured;$liveGrid.ClearSelection();foreach($row in @($liveGrid.Rows)){if(-not $row.IsNewRow){$row.Selected=$true}};$liveStatus.Text="Сценарий: $($captured.Count) целей TCP/UDP"}else{$liveStatus.Text='Новых соединений нет'}}|Out-Null
...
New-Button $liveTop 'Все TCP+UDP' 230 50 125 {$liveGrid.SelectAll()}|Out-Null
New-Button $liveTop 'Снять выбор' 365 50 110 {$liveGrid.ClearSelection()}|Out-Null
...
$refreshLearning={$state=InvokeAdaptiveLearningStatus;if(-not $state.Active){$learnTitle.Text='Подбор не запущен';$learnDetail.Text='';&$setLearnStage 'Запишите действие во вкладке «Приложения», затем запустите подбор всего сценария.' 0 $false;return};$percent=[int](($state.Index-1)*100/[Math]::Max(1,$state.Total));$learnTitle.Text=('Flowseal {0}/{1}: {2}' -f $state.Index,$state.Total,$state.Profile);$learnDetail.Text=('Приложение: {0}    Сценарий: {1}/{2}`r`nДиагностика winws: строк {3}, распознано {4}, hostlist {5}, desync {6}' -f $state.Process,$state.Target,$state.Protocol,$state.DebugLines,$state.Recognized,$state.HostChecks,$state.DesyncEvidence);&$setLearnStage ('Выполните проверяемое действие. «Работает» принимается только при наличии диагностических событий winws.') $percent $true}
...
$liveTimer=New-Object Windows.Forms.Timer;$liveTimer.Interval=1000;$liveTimer.Add_Tick({if($liveRefreshBusy){return};if($tabs.SelectedTab -eq $traffic -and $appTabs.SelectedTab -eq $livePage){$liveRefreshBusy=$true;try{&$refreshLive}finally{$liveRefreshBusy=$false}}elseif($tabs.SelectedTab -eq $learn){&$refreshLearning}})
